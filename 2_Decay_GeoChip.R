...
#################### Altitude (HKV)
prefix='HKV'
project = project_HA_GeoChip #%>% subset_samples(.,! Site %in% c('C','D','E','F'))
OTU <- otu_table(project)
ENV <- sample_data(project) #%>% as.data.frame(.) 
#################### 
jaccard = vegdist(t(OTU), method="jaccard", binary=FALSE) %>% col3m(.,dist_name ='jaccard')
...
SDecay <- ggplot(decay_data,aes(x=ln.distance,y=Similarity.bray))+
  geom_point(alpha = 0.5,stroke = 0.3,size=1.5,shape=21,aes(fill=Study))+geom_smooth(method='lm',aes(color=Study),se = F,linetype =2)
SDecay <- ggplot(decay_data,aes(x=ln.distance,y=Similarity.bray))+
  geom_point(alpha = 0.5,stroke = 0.3,size=1.5,shape=21,aes(fill=Study))#+geom_smooth(aes(color=Study),se = F)
SDecay+PTs+ylim(c(0.4,1))+facet_grid(~Study,scales = 'free')#+lm_Site+Ps_Sitescm_Site+sfm_Site
