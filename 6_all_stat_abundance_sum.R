...
gene_filter_pack = function(project){
  ...
  for (j in 1:ncol(Env_fun)) {
    ...
    for (i in 1:length(unique_derep_level)) {
      # extract subset for sum
      family.index = names(derep_level)[derep_level==unique_derep_level[i]]
      # get abundance sum of OTU
      gene.abund = OTU[family.index,]
      if (length(family.index)>1) {
        gene.abund=colSums(gene.abund,na.rm = T)
      }
      # get match index for env data if there is NA in OTU
      match.index = !is.na(gene.abund)
      # match and test (>6 for correlation analysis)
      if (sum(match.index)>6) {
...
write.csv(dt_stat,file = paste0(folder,'/Abund-sum_detail_',prefix,'_',note,'.csv'),row.names = FALSE)
group_stat = dt_stat %>% group_by(ENV_fun,gene_name) %>% summarise(n=n())
write.csv(group_stat,file = paste0(folder,'/Abund-sum_group-stat_',prefix,'_',note,'.csv'),row.names = FALSE)
